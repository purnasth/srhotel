...
class Features extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'title', 'parentId', 'fa_icon', 'image', 'brief', 'status', 'sortorder', 'added_date');
	...
	static function get_by_id($id=0){
		global $db;
		$result_array = self::find_by_sql("SELECT title, fa_icon, image FROM ".self::$table_name." WHERE id={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
	}

	static function get_all_byparnt($parentId=0){
		global $db;
		$sql = "SELECT * FROM ".self::$table_name." WHERE parentId=$parentId AND status=1 ORDER BY sortorder DESC";
		return self::find_by_sql($sql);
	}
