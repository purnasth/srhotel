			...
			//print_r($subaRec);
	    		if(!empty($subaRec)) {
					...
					foreach($subaRec as $subaRow) {	
						...
						if(!empty($imgbulk[0])){$imglink=$imgbulk[0];
		        	$file_path = SITE_ROOT.'images/subpackage/banner/'.$imgbulk[0];
			        	...
			        	if(file_exists($file_path) && !empty($imgbulk[0])) {
							...
							$actlist.='<div class="item-room">
											...
											<p>'.$subaRow->onep_price.' '.$subaRow->threep_price.' <br /><span>Per Night</span></p>
											</div>
											</div>
											</div>
											';
...
$pkgbnn=''; $pkgbdcrm=''; $pkgresult = ''; $resubpkg=''; $roomlist=''; $hroom='';
